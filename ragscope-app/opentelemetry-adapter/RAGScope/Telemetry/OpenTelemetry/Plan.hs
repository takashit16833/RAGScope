...
module RAGScope.Telemetry.OpenTelemetry.Plan (
  OwnerPlans (..),
  Seed (..),
  Transfer (..),
  Chain (..),
...
data OwnerPlans resource = OwnerPlans
...
data Chain before after where
  Done :: Chain resource resource
  (:>>) ::
...
data Plan result where
  PurePlan :: result -> Plan result
  ApPlan :: Plan (a -> result) -> Plan a -> Plan result
  Resource :: Build result -> Plan result

instance Functor Plan where
  fmap :: (a -> b) -> Plan a -> Plan b
  fmap f = ApPlan (PurePlan f)
