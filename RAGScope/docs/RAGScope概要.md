...
質問に対して取得された文書断片と、実際に回答生成へ使われた文脈を区別して保存する。回答だけでなく、その回答へ至る過程を確認できる。
...
- Markdown / TXT形式の技術文書を取り込み、検索に使用できる。
- キーワードによる検索と意味的な検索を実行し、条件の異なる結果を比較できる。
- 検索候補の統合や並べ替えを行い、回答生成へ渡す文脈を選択できる。
- 自分の環境で動かすモデルを使用して、回答と引用を生成できる。
- 検索、回答、引用、回答可能性、処理時間などを評価できる。
- 実験条件と質問ごとの生データを保存し、評価指標を再集計できる。
- 実験結果をMarkdown / CSV形式のレポートとして出力できる。
- APIまたはCLIから主要な処理を実行できる。
...
最初は英語文書を対象とし、日本語文書は第2のコーパスとして扱う。RAG本体はローカル環境で構築し、外部のLLM APIへ依存せず、自分の環境で動かす学習済みモデルを中心に検証する。

完成形は大規模なWebサービスではなく、API、CLI、実験データ、読みやすいレポートによって、検索と評価の過程を確認できるローカル実行可能なシステムとする。

## 7. 対象外・主張しないことの概要

初期版からv1.0までは、次を対象外とする。

- PDF、OCR、画像、複雑なレイアウトの解析
- 大量データや大規模トラフィックへの対応
- 複数箇所の根拠を統合するmulti-hop評価
- マルチテナント、本格的なWeb UI、認証、ストリーミング
- エージェント機能
- Kubernetes、分散推論、高可用構成
- 本番環境としての長期的なクラウド運用
