...
v0.0では、文書読み込み、チャンク化、AI推論サービスとの通信、PostgreSQL処理、dense検索など、異なる処理を1つのローカル実行として制御する。
...
本設計は、[v0.0 — 最小のdense検索を実行できる](<../../project-management/milestones/v0.0/v0.0.md>)で実装するRAGScopeアプリケーション側の共通エラー・構造化ログ基盤と、それを利用する各機能へ適用する。


v0.0では、次を扱う。


- RAGScopeアプリケーション内部で使用する共通エラーの意味と変換境界
- CLI実行を単位とする実行Context
- RAGScopeアプリケーションが出力するapplication eventの共通構造
...
| 表現 | 責務 | 含める情報 | 含めない情報 |
|---|---|---|---|
| 共通エラー | RAGScopeアプリケーション内部で失敗を正常結果と区別して伝播する | category、安定したcode、安全なmessage、内部原因、error固有context | CLIの装飾、HTTP status、ログEnvelope |
...
| RAGScopeアプリケーション → AI推論サービス | operationの意味、入力、RAGScopeアプリケーション側の実行Contextを管理する | AI推論サービス側は共有された失敗の意味をresponseへ変換する。正確なschemaはAPI設計・OpenAPIで定義する |


RAGScopeアプリケーションのbusiness logicは、stderr、CloudWatch API、具体的なlogger library、JSON encoderへ直接依存しない。
...
| `dependency` | AI推論サービス、model、PostgreSQLなど依存componentのoperationが失敗した | service unavailable、DB query失敗 |
...
## 9. RAGScopeアプリケーションとAI推論サービスの失敗契約


RAGScopeアプリケーションとAI推論サービスは、失敗について少なくとも次の意味を共有する。
...
- 外部へ提示してよい安全なmessage
- どのoperationで失敗したか
- 同じRAGScopeの実行へ関連付けるための相関情報


境界規則：


- Pythonのexception class、stack trace、library固有messageをRAGScopeアプリケーションへそのまま公開しない。
- AI推論サービス側は、予期した失敗を共有error表現へ変換する。
- RAGScopeアプリケーション側は、共有codeを認識できる場合は意味を保って共通エラーへ変換する。
- responseをdecodeできない、codeを認識できない、接続自体に失敗した場合は、RAGScopeアプリケーション側の境界errorとして分類する。
- RAGScopeアプリケーション側の`execution_id`はCLI実行の相関情報として保持する。HTTPでの具体的なheader、body field、request IDはAPI設計・OpenAPIで決定する。
- HTTP statusとerror category・codeの正確なmappingはAPI設計を正本とする。


v0.0では、RAGScopeアプリケーションがAI推論サービスへrequestを送る前後のeventを同じ`execution_id`で記録できればよい。process間の正確なSpan親子関係は定義しない。
...
sequenceDiagram
    ...
    participant App as RAGScopeアプリケーション
...
- RAGScopeアプリケーションとAI推論サービスが共有するerror response schemaをOpenAPIで定義する。
...
- AI推論サービスのerror responseとOpenAPI
