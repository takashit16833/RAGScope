...
| Haskell → AI推論サービス | operationの意味、入力、Haskell側の実行Contextを管理する | AI推論サービス側は共有された失敗の意味をresponseへ変換する。正確なschemaはAPI設計・OpenAPIで定義する |
...
## 9. HaskellとAI推論サービスの失敗契約


HaskellとAI推論サービスは、失敗について少なくとも次の意味を共有する。
...
v0.0では、HaskellがAI推論サービスへrequestを送る前後のeventを同じ`execution_id`で記録できればよい。process間の正確なSpan親子関係は定義しない。
...
- HaskellとAI推論サービスが共有するerror response schemaをOpenAPIで定義する。
