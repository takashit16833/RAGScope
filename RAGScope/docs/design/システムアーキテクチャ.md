...
| Python AI Service | モデルとTokenizerのロード、Embedding生成、reranking、回答生成、token数計測、モデル固有の推論パラメータ、AIライブラリを利用する回答評価、AI推論の処理時間計測、共通契約に従った処理結果と失敗情報の受け渡し |
...
- 共通エラー契約は、外部処理を含む失敗を正常結果と区別し、各インターフェースで一貫して扱える境界を定める。
- 構造化ログは、アプリケーションイベントの意味と出力先を分離し、具体的なsinkをbusiness logicへ埋め込まない。
- HaskellとPython AI Serviceは、処理結果と失敗情報について共通契約に従う。
...
v0.0への段階的な導入範囲とretry / timeoutの共通原則は、[ADR-0001 — 共通実行基盤をv0.0で段階的に導入する](<../adr/ADR-0001 共通実行基盤をv0.0で段階的に導入する.md>)を参照する。正確なエラー表現、ログschema、相関情報、operation固有のtimeout・retry条件、型、設定、DB制約は、機能別設計、コード、OpenAPI、migration、テストなどを正本とする。
...
- Haskellのapplication処理は、共通エラー・ログの契約へ依存し、具体的なログsinkをbusiness logicへ埋め込まない。
...
共通エラー・ログ契約とretry / timeoutの共通原則はv0.0で定義する。各APIの正確なrequest / response、batch上限、timeout、HTTP statusとエラーの対応、retryの適用条件、モデル未ロード時の挙動は、そのAPIへ着手する段階で機能別設計として具体化する。正確なschemaはOpenAPI等の機械可読な定義を正本とする。
...
| エラー・ログ設計 | 共通エラー契約、構造化ログ、相関情報、出力先との分離、将来のtracingへ拡張できる境界 |
