...
| Python AI Service | モデルとTokenizerのロード、Embedding生成、reranking、回答生成、token数計測、モデル固有の推論パラメータ、AIライブラリを利用する回答評価、AI推論の処理時間計測、共通契約に従った処理結果・失敗情報・application eventの受け渡し |
...
共通エラー・ログ契約とretry / timeoutの共通原則はv0.0で定義する。1回のCLI commandからPython AI Serviceを呼び出す場合は、同じ`execution_id`をHTTP境界で伝播する。各APIの正確なrequest / response、相関情報の伝播方法、batch上限、timeout、HTTP statusとエラーの対応、retryの適用条件、モデル未ロード時の挙動は、そのAPIへ着手する段階で機能別設計として具体化する。正確なschemaはOpenAPI等の機械可読な定義を正本とする。
...
| API設計 | Haskell APIとPython AI Serviceの契約、HTTP固有のエラー表現、timeout、batch |
| エラー・ログ設計 | 共通エラー契約、構造化ログ、JSON Schema、相関情報、execution・service・diagnostic eventの境界、logger失敗時の意味、出力先との分離、将来のtracingへ拡張できる境界 |
| リトライ・タイムアウト設計 | HaskellからPython AI Serviceへ文書チャンクのEmbeddingを要求するoperationに必要なtimeout、retry対象、再試行安全性、operation固有の実行制御 |
