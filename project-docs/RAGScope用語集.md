...
この用語集では、ドメインモデルで定義した「文書」「評価データ」「検索・回答生成」「実験・評価」の4領域に沿って、各領域で使用する正式用語と概念の意味を定義する。RAGScopeアプリケーション、AI推論サービス、RAGScope API、RAGScope CLI、Invocation、処理失敗、内部処理の失敗、最終失敗は、ドメイン領域とは別のシステム上の用語として扱う。

### 主要概念の関係

次の図は、4つのドメイン領域にまたがる主要な概念どうしの関係を示す。各検索方式や回答生成用文書チャンク選択など、領域内の詳しい処理は後続の各節で示す。RAGScopeアプリケーションなどのコンポーネント構成と通信経路は[システムアーキテクチャ](./design/システムアーキテクチャ.md)を正本とする。
...
flowchart LR
    ...
    subgraph RetrievalGeneration["検索・回答生成"]
        Retrieval["検索"] -->|"回答生成用文書チャンク選択・<br>回答生成用プロンプト構築"| Generation["回答生成"]
    end

    subgraph ExperimentEvaluation["実験・評価"]
        Conditions["実験条件"] -->|使用| Experiment["実験"]
        Experiment -->|結果| Result["実験結果<br>3種類の結果の総称"]
        Result -->|含む| Execution["評価データごとの実行結果"]
        Result -->|含む| Evaluation["評価データごとの評価結果"]
        Result -->|含む| Aggregate["実験全体の集計結果"]
        Execution -->|評価| Evaluation
        Execution -->|集計| Aggregate
        Evaluation -->|集計| Aggregate
        Result -->|出力| Report["実験レポート"]
    end

    Evidence -->|"文書内の範囲を参照"| Version
    Question -->|入力| Retrieval
    SearchData -->|検索に使用| Retrieval
    Dataset -->|使用| Experiment
    Data -->|対象| Execution
    Data -->|評価基準| Evaluation
    Experiment -->|"評価データごとに実行"| Retrieval
    Retrieval -->|"検索で得た情報"| Execution
    Generation -->|"回答生成で得た情報"| Execution
```

`実験結果`は、`評価データごとの実行結果`、`評価データごとの評価結果`、`実験全体の集計結果`の3種類をまとめた総称である。
...
| [評価データごとの実行結果](#評価データごとの実行結果) | 1件の評価データについて、検索・回答生成を実行して得た結果 |
| [評価](#評価) | 1件の評価データについて、検索・回答生成で得られた結果を評価し、評価データごとの評価結果を得る処理 |
| [評価データごとの評価結果](#評価データごとの評価結果) | 1件の評価データについて、実行結果などを評価して得た指標や判定 |
| [実験全体の集計結果](#実験全体の集計結果) | 評価データごとの実行結果と評価データごとの評価結果を実験単位で集計した結果 |
| [実験結果](#実験結果) | 評価データごとの実行結果、評価データごとの評価結果、実験全体の集計結果の総称 |
| [実験レポート](#実験レポート) | 保存済みの実験結果や比較結果を人が確認できる形へ出力した成果物 |

**システム**

RAGScopeを構成するコンポーネント、利用者向けインターフェース、アプリケーションの実行境界と失敗に関する用語をまとめる。
...
| [処理失敗](#処理失敗) | UseCaseまたは内部処理が、処理の契約として具体的な失敗型の値で呼び出し元へ伝える失敗 |
| [内部処理の失敗](#内部処理の失敗) | あるUseCaseが結果を作るために内部で呼び出す処理の処理失敗 |
| [最終失敗](#最終失敗) | 実行制御を経ても結果を作れなかったときに呼び出し元へ伝える処理失敗 |

## 1. システム

この節では、4つのドメイン領域を実現または利用するシステム上のコンポーネント、インターフェース、実行境界、失敗に関する用語を定義する。
...
UseCaseまたはUseCase内部で呼び出す処理が、その処理の契約として具体的な失敗型の値で呼び出し元へ伝える失敗。

async exception、意図された中断・キャンセル、process startup / shutdownの失敗は処理失敗に含めない。処理失敗をHaskellで`Either`、`ExceptT`、その他のどの表現として返すかは、この用語では固定しない。

### 内部処理の失敗

あるUseCaseが定義した結果を作るために、そのUseCase内部で呼び出す処理の[処理失敗](#処理失敗)。

ここでいう「内部処理」は、そのUseCaseから見た呼び出し関係を表す。RAGScope全体へ固定のlayer hierarchyを追加する用語ではない。内部処理の失敗が発生しても、retryやfallbackなどによってUseCaseが最終的に定義した結果を作れた場合、その失敗はUseCaseの[最終失敗](#最終失敗)ではない。

### 最終失敗

UseCaseまたは内部処理に適用されるretry、timeout、fallback、継続・終了の制御を経ても、その処理が定義した結果を作れなかったときに、呼び出し元へ伝える[処理失敗](#処理失敗)。

最終失敗は処理境界ごとに決まる。同じ失敗が、ある内部処理にとっては最終失敗でも、その呼び出し元がretryやfallbackで回復した場合は、呼び出し元の処理の最終失敗にはならない。
...
評価データセットから選択した評価データと実験条件を使ってRAGの処理を行い、評価データごとの実行結果、評価データごとの評価結果、実験全体の集計結果を保存・比較する単位。
...
1件の評価データについて、1つの実験の中で検索・回答生成を実行して得た結果。

各検索段階の結果、回答生成用文書チャンク、回答生成用プロンプト、回答、引用、処理時間、状態、エラーなどを、その評価データと実験に関連付けて扱う。

### 評価

1件の評価データについて、検索結果、回答生成用文書チャンク、回答、引用など、検索・回答生成で得られた結果を評価し、評価データごとの評価結果を得る処理。

評価では、その評価データが持つ参照回答、回答可能性、正解根拠などを必要に応じて使い、定めた評価指標や判定規則に従って結果を調べる。

複数の評価データについて得られた結果をまとめ、実験全体の値を求める処理は集計であり、評価とは区別する。

### 評価データごとの評価結果

1件の評価データについて、その評価データに対応する評価データごとの実行結果を評価して得た指標や判定。

実験全体の集計結果とは区別する。

### 実験全体の集計結果

1つの実験について、評価データごとの実行結果と評価データごとの評価結果を集計して得た結果。

評価指標の集計に加え、処理時間、成功率、タイムアウト率など、評価データごとの実行結果から集計する値も含む。

### 実験結果

1つの実験で得られた結果の総称。

評価データごとの実行結果、評価データごとの評価結果、実験全体の集計結果を含む。
