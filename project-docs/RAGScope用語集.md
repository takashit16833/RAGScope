...
この用語集では、ドメインモデルで定義した「文書」「評価データ」「検索・回答生成」「実験・評価」の4領域に沿って、各領域で使用する正式用語と概念の意味を定義する。RAGScopeアプリケーション、AI推論サービス、RAGScope API、RAGScope CLI、handlerは、ドメイン領域とは別のシステム上の用語として扱う。
...
RAGScopeを構成するコンポーネント、利用者向けインターフェース、およびそれらとUseCaseの境界に関する用語をまとめる。
...
| [handler](#handler) | RAGScope API / CLI固有の入力・出力とUseCaseの入力・結果を仲介するRAGScopeアプリケーション側の境界処理 |

## 1. システム

この節では、4つのドメイン領域を実現または利用するシステム上の用語を定義する。
...
RAGScope API / RAGScope CLIとUseCaseの間で、インターフェース固有の入力・出力とUseCaseの入力・結果を仲介するRAGScopeアプリケーション側の境界処理。

RAGScope API / RAGScope CLIから受け取った入力をUseCaseへ渡せる値へ変換してUseCaseの実行へつなぎ、UseCaseから制御が戻った後は、その結果をCLI表示やHTTP responseなどのインターフェース固有の出力へ変換する。UseCaseそのものはhandlerに含めない。

handlerは独立したコンポーネントではなく、`handler`という名称だけでは単一のmodule、function、または呼び出し回数を意味しない。具体的な責務配置は[システムアーキテクチャ](./design/システムアーキテクチャ.md)を正本とする。
