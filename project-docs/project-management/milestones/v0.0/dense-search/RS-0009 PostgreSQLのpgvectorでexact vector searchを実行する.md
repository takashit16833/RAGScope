...
- [ ] 質問Embeddingのベクトル次元がDB schemaの次元と一致しない場合を、正常な検索と区別できる具体的なerrorとして扱える
- [ ] PostgreSQLへ接続できない場合とSQL実行に失敗した場合を、正常な検索と区別できる具体的なerrorとして扱える
- [ ] PostgreSQL処理に適用できるOpenTelemetry DB Semantic Conventionと標準計装を優先し、同じDB処理を表す独自Span / EventRecordを重複して追加していない
