...
- [ ] 引数がない場合、空文字、または空白だけの質問文を、正常な検索実行と区別できる入力errorとして扱える
...
- [ ] 入力error、AI推論サービスへの接続失敗、質問Embedding生成失敗、PostgreSQL接続失敗、検索失敗を、利用者が原因の種類を確認できる表現へ各具体的なerrorから変換し、標準エラー出力へ表示できる
...
- [ ] CLIの引数解析、処理の呼び出し順序、検索結果の整形、主要なerror表示を自動テストで確認できる
...
- [ ] v0.0をローカル環境で再実行するための最小限のsetup、起動、文書取り込み、検索コマンド、終了方法が`README.md`へ記載されている

### 正本と整合

- [ ] CLIの正確なcommand、引数、終了状態を実装と`--help`から確認できる
...
- Web UI、REST API、認証、streaming
- 質問、質問Embedding、検索結果、実行履歴の永続化
- 検索結果を回答生成用文書チャンクとして選択する処理
- 生成モデルによる回答生成と引用
...
- v0.0では質問文をコマンド引数として1件受け取る。正確なcommandと引数は実装と`--help`を正本とする。
- 正常な検索結果は標準出力、入力errorや外部service・DBの失敗は標準エラー出力へ表示する。
- error表示には内部Exceptionや認証情報をそのまま露出させず、利用者が失敗した処理段階を識別できる情報を含める。
...
- 関連文書チャンクが上位に含まれることは、v0.0の経路が意味的な検索として接続されたことを確認するsmoke testとし、検索品質が十分または最適であるとは主張しない。
