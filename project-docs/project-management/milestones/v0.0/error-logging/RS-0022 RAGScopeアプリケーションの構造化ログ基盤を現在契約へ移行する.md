...
- Observability公開APIの正確な型・module分割と、`withTrace` / `withSpan`がTracingの`observeResult`をどの境界で組み合わせるかは、今回確定したTracing Portの契約を維持したうえで引き続き設計・実装する。
...
- [ ] Application / FeatureはTracingを直接利用せずObservability / Loggingを通して必要な処理を行う
