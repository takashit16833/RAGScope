...
[ADR-0002](<../../../../adr/ADR-0002 共通実行基盤の契約とコンポーネント実装を分離する.md>)では、retryを一時的失敗かつ再試行安全な場合だけ適用し、operation固有のPolicyと実装を具体的な利用箇所へ着手する段階で決定する方針を採用している。

このTicketでは、RAGScopeアプリケーションからAI推論サービスへ文書チャンクのEmbeddingを要求するoperationを最初の対象として、必要なretry / timeoutの現在設計を`design/リトライ・タイムアウト設計.md`に定義する。RS-0012で設計したAPI契約と主要な失敗を基に、どの失敗を何の根拠で再試行するか、再実行しても安全な条件は何か、どの時間範囲を制限するかを明確にし、RS-0017とRS-0004が実装へ着手できる判断基準を整える。
...
- `design/Embedding生成設計.md`と文書チャンクのEmbedding生成APIのOpenAPI初期定義から、operationの境界と主要な失敗を確認できる
