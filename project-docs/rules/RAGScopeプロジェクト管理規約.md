...
- EpicをTicketへ具体化するときは、個別Ticketを詳細化する前に、Epic全体のTicket構成、各Ticketの主な成果物、依存関係、実行順序、作成または更新する要求・設計書・ADR・Experiment・機械可読な定義を確認する。
...
- 初期設計は、後続Ticketが実装へ着手できる見通しと判断基準を示す。実装前に型、SQL、API schemaなどの正確な詳細をすべて固定することは求めず、実装で具体化または変更された現在設計は、各Ticketで同じ設計書へ反映する。
- 既存の設計書が必要な判断基準をすでに提供している場合や、Epicが小規模で専用の設計Ticketを分ける合理性がない場合は、設計Ticketを機械的に追加しない。
- Ticket構成は着手前の計画であり、実装や検証によって新しい事実が判明した場合は、影響範囲を確認したうえで必要な見直しを行ってよい。
...
GitHub Releaseには、実現した能力、対象要求、確認方法、利用方法、既知の制約を必要な範囲で記載する。対象要求はMilestoneノートの要求IDとリンクを再利用し、要求本文を複製しない。
