...
- フロー図の要素名だけでは、入力、処理、出力のいずれを表すか、または何を行う段階かが分からない場合は、要素名を具体化するか、図の直後に短い説明を置く。具体的な値やアルゴリズムを図へ詰め込まず、詳細の正本へリンクする。
- システム全体で採用済みの処理段階、現在のMilestoneで具体化済みの規則、後続設計へ委ねる未決定事項を区別する。未決定の選択肢を、採用済みの仕様と同じ断定表現で記載しない。
- 長い段落が続き、論点や情報の階層を追いにくい場合は、見出し、表、図、コード例、Calloutで構造を示せないか検討する。文章を機械的に分割したり、すべてを表へ変換したりしない。
...
- 説明文は日本語を基本とする。APIパス、コード上の識別子、製品名、規格名、機械可読な定義の項目名、原表記によって意味を区別する必要がある技術用語は、原表記を使用してよい。
- 一般的な英語の設計用語を日本語文中へ置くだけでは意味が伝わりにくい場合は、対象、動作、結果が分かる具体的な日本語へ置き換える。原表記を残す必要がある場合は、初出時に日本語で意味を説明する。
