...
利用インターフェースが受け付けた1回のトップレベルな操作全体にも、ユースケース実行とは別に成功・失敗の境界を持たせる。この外側の実行結果を、この設計では概念上`OperationResult`と呼ぶ。`OperationResult`は`Success`または`Failure`のいずれかであり、ユースケースの`Left failure`は操作全体が失敗する原因の1つとして外側の`Failure`へ引き渡す。routing・command判定、入力の変換・検証、ユースケース終了後の結果処理で発生した通常の失敗も、Feature固有failureへ変換せず、対応する外側の`Failure`として扱う。
