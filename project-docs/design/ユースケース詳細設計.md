...
> [!abstract] この文書の役割
> [ユースケース基本設計](./ユースケース基本設計.md)で定義する`UseCaseResult`を、RAGScopeアプリケーションのHaskell実装でどのように表現し、ユースケース境界から受け渡すかを定義する。各ユースケースで定義する具体的な失敗のHaskell表現、そのfailure値に`toErrorType`を適用して`ErrorType`を得る規則、実行に必要な依存能力の所有と受け渡しも扱う。
>
...
Applicationのcomposition rootは、Observability、UseCase別Logger、その他そのUseCaseが必要とする具体的な能力を組み立て、UseCase所有の依存recordを構築してUseCaseへ渡す。UseCase別Loggerの組み立ては[RAGScopeアプリケーション構造化ログイベント変換詳細設計](./logging/RAGScopeアプリケーション構造化ログイベント変換詳細設計.md)に従う。
...
UseCase固有failure型に対する`ToErrorType` instanceは、そのfailure型を定義する`RAGScope.<UseCase>.Failure`へ置く。UseCase側がinstanceとして変換規則を提供することと、UseCase本体が実行時に`toErrorType`を適用することは別の責務である。UseCase本体はfailure値を返し、`ErrorType`を必要とする処理がそのfailure値へ`toErrorType`を適用する。UseCase固有failure型自身には、failureであることだけを理由に構造化ログ用の変換を要求しない。

ユースケースの失敗を構造化ログへ記録する場合は、その失敗を表すUseCase eventをUseCase側で定義し、そのeventから`LogSpec`への純粋変換を提供する。Applicationのcomposition rootは、その変換とLogging Runtimeが作る`Logger m LogSpec`を`contramap`で合成し、UseCaseが使う`Logger m <UseCaseEvent>`を作る。この変換境界は[RAGScopeアプリケーション構造化ログイベント変換詳細設計](./logging/RAGScopeアプリケーション構造化ログイベント変換詳細設計.md)、ユースケース失敗イベントを関連付ける`span`と同じ`error_type`を共有する規則は[実行追跡・構造化ログ契約設計](./実行追跡・構造化ログ契約設計.md)を正本とする。
