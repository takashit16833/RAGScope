...
> [!abstract] この文書の役割
> [ユースケース基本設計](./ユースケース基本設計.md)で定義するユースケースの実行境界、成功、失敗と、各[機能設計](./features/README.md)で定義する具体的な失敗を、RAGScopeアプリケーションのHaskell実装でどのように表現し、ユースケース境界で受け渡すかを定義する。
>
> 個々のユースケースで何を成功・失敗とみなすかは基本設計、具体的にどのような失敗があるかは各機能設計を正本とする。正確なHaskell型・constructor・関数定義は実装コードを正本とし、この文書では実装が守る表現と受け渡しの契約を扱う。
...
`ErrorType`を必要とする公開境界は、具体的なFeature failure型へ固定せず、`ToErrorType`を満たすfailureを受け取り、その境界内で`toErrorType`を適用する。変換より下位の処理にはFeature固有failureではなく`ErrorType`を渡す。root `span`へユースケース失敗を反映する場合はObservability Runnerがこの変換境界を担当し、Observabilityの下位処理とTracingには変換後の`ErrorType`を渡す。
...
ユースケースの失敗を構造化ログへ記録する場合は、その失敗を表すFeature eventとして記録する。Feature eventは`ToLogSpec`によってLoggingの`LogSpec`へ変換される。

```text
Feature failure ── ToErrorType ──> ErrorType

Feature failureをログへ記録する場合
        ↓
   Feature event ── ToLogSpec ──> LogSpec
```

したがって、Feature failureを追加しただけでは構造化ログイベントは追加されない。失敗をログへ記録するFeatureは、その失敗を表すFeature eventと、Feature eventからLogging共通表現へ変換する契約を別に定義する。

ユースケース失敗イベントを記録する境界とroot `span`との`error_type`共有規則、Feature eventを構造化ログへ接続するときに定義する情報は[実行追跡・構造化ログ契約設計](./logging/実行追跡・構造化ログ契約設計.md)を参照する。具体的なFeature eventとその属性は各機能設計、正確な`ToLogSpec` class・instanceとLoggingへの受け渡しAPIは実装コードを正本とする。
