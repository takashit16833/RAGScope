...
UseCaseは`Either failure result`として自身の成功・失敗を返すまでを責務とし、`failure`を`ErrorType`、構造化ログ、Tracingなどの観測用表現へ変換しない。利用者操作も失敗として終了する場合、Applicationは同じ`failure`値と、そのfailure所有側が提供する対応`ErrorClassifier failure`を共通`OperationFailure`で包んで返す。具体的なfailure型に応じたApplication側の処理は、利用インターフェースのhandlerなど、そのfailure値を具体型のまま受け取れる場所で必要に応じて行う。
...
この依存recordはUseCase実行に必要な能力を表すものであり、利用者操作全体の依存を表すものではない。Application composition rootはこの依存recordをUseCase実行関数へ適用し、依存を閉じた実行関数を利用インターフェースのhandlerへ渡す。handlerはUseCase内部の依存構造を知らず、UseCaseへ入力を渡して結果を受け取る能力だけを利用する。Operationは機能固有の中間層ではなく、handlerが担当する利用者操作全体を`withOperation`で囲む境界として扱う。詳細は[利用者操作詳細設計](./利用者操作詳細設計.md)を正本とする。

現在のRAGScopeではApplication全体の長寿命な依存を保持する`AppEnv`は導入しない。UseCaseはApplication全体の環境を直接受け取らず、所有する依存recordから自身に必要な能力だけを受け取る。利用インターフェースのhandlerにも、Application composition rootからそのhandlerが必要とする能力だけを渡す。
