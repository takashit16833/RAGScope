...
この依存recordはUseCase実行に必要な能力を表すものであり、利用者操作全体の依存を表すものではない。Operation側も同じ原則で、そのOperationが実行時に必要とする能力だけをOperation固有の依存recordへまとめる。ただしOperationへUseCase所有の依存recordそのものは渡さず、Application composition rootがUseCase依存recordをUseCase実行関数へ適用し、依存を閉じた実行関数をOperation依存recordへ格納する。OperationはUseCase内部の依存構造を知らず、UseCaseへ入力を渡して結果を受け取る能力だけを利用する。Operation依存recordの詳細は[利用者操作詳細設計](./利用者操作詳細設計.md)を正本とする。

現在のRAGScopeではApplication全体の長寿命な依存を保持する`AppEnv`は導入しない。OperationとUseCaseはいずれもApplication全体の環境を直接受け取らず、所有する依存recordから自身に必要な能力だけを受け取る。
