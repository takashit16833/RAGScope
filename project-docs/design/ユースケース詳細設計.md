...
ユースケース失敗イベントを記録する境界とroot `span`との`error_type`共有規則は[実行追跡・構造化ログ契約設計](./logging/実行追跡・構造化ログ契約設計.md)を参照する。具体的なFeature eventとその属性は各機能設計、正確な`ToLogSpec` instanceとLoggingへの受け渡しAPIは実装コードを正本とする。
