...
> [!abstract] この文書の役割
> [ユースケース基本設計](./ユースケース基本設計.md)で定義するユースケースの実行境界・成功・失敗と、それを必要な場合だけ内包する1回の利用者操作全体の成功・失敗境界との関係を、RAGScopeアプリケーションのHaskell実装でどのように表現し受け渡すかを定義する。各[機能設計](./features/README.md)で定義する具体的なユースケース失敗を、ユースケース境界でどう表現するかも扱う。
>
...
[ユースケース基本設計](./ユースケース基本設計.md)で定義する1回の利用者操作全体の成功・失敗境界は、Haskell実装でもユースケース実行の`Either`を必要な場合だけ内包する外側の実行結果として表す。この外側の実行結果を、この設計では概念上`OperationResult`と呼ぶ。`OperationResult`は`Success`または`Failure`のいずれかであり、ユースケースが実行された場合、その`Right`または`Left`はOperation全体の結果を決める内側の結果として扱う。ユースケースの`Left failure`は外側の`Failure`へ引き渡す原因の1つとなる。routing・command判定、入力の変換・検証、ユースケース終了後の結果処理で発生した通常の失敗も、Feature固有failureへ変換せず、対応する外側の`Failure`として扱う。

```text
1回の利用者操作
├─ routing / command判定 / 入力処理
├─ UseCase（必要な場合）
│    └─ Either FeatureFailure FeatureResult
│         ├─ Right result
│         └─ Left failure
├─ UseCase終了後の結果処理
└─ 操作全体の最終結果
...
UseCaseを実行して`Left failure`となった場合、Operation全体も`Failure`となる。一方、UseCaseが`Right result`となったことだけではOperation全体の`Success`は確定せず、UseCase前後を含む操作固有処理が完了して利用者が依頼した結果が成立した時点で`Success`となる。UseCaseを呼び出さずに入力処理などで終了するOperationもある。
