...
- RAGScopeアプリケーションをシステムの司令塔とし、文書、検索、評価、実験の流れを制御する。RAGScopeアプリケーションはHaskellで実装する。
...
検索処理とモデルに関する追加の正式用語は、次節以降で定義する。
...
`execution_id`と`event_id`はUUIDv4とする。将来の永続IDは、保存・検索要件を具体化するデータモデル設計でUUIDv7を含めて選定する。正確なJSON、HTTP、DB、コード上の制約は、それぞれJSON Schema、OpenAPI、migration、型とテストを正本とする。
...
flowchart LR
    ...
    Application <-->|HTTP / JSON| Python["AI推論サービス"]
...
利用者はRAGScope API / RAGScope CLIを通じてRAGScopeを操作する。RAGScopeアプリケーションはPostgreSQLとAI推論サービスを組み合わせ、文書取り込み、検索、回答生成、評価、実験管理、レポート生成を実行する。
...
| コンポーネント | 主な責務 |
|---|---|
| RAGScopeアプリケーション | 利用者向けRAGScope API・RAGScope CLI、処理全体の制御、文書と評価データの管理、文書分割、検索、順位統合、context選択、prompt構築、基本評価指標、実験管理、レポート生成、設定、ログ、timeout / retry、テスト |
...
| PostgreSQL | 正規化済み文書、文書チャンク、Embedding、評価データ、実験条件、検索結果、回答、引用、評価結果、実行状態の永続化、全文検索、vector検索 |
...
AIモデル固有のロード方法や推論実装はRAGScopeアプリケーションへ持ち込まない。

### 3.2 AI推論サービスの責務境界

AI推論サービスは、AIモデルとAIライブラリに依存する処理を提供する。

- 文書チャンクのEmbeddingまたは質問Embeddingを生成する。
...
- `/embeddings`は文書チャンクのEmbeddingと質問Embeddingの生成に使用する。
...
4. 文書分割条件に従って文書チャンクを生成する。
5. 各文書チャンクについて、元文書、文書バージョン、元文書内の位置を保存する。
6. AI推論サービスで各文書チャンクのEmbeddingを生成する。
7. 文書チャンクとEmbeddingを対応付けて保存する。

正規化、チャンク化、文書チャンクのEmbedding生成は、文書または分割条件を登録・更新したときに行い、質問処理のたびには実行しない。

正規化、offset、文書チャンクの不変条件、Embedding入力の組み立ては、実装に必要となった段階で文書処理の機能別設計として定義する。
