...
RAGScopeアプリケーションからAI推論サービスへ処理を依頼するときは、現在のOpenTelemetry Trace Contextを通信境界へ伝播し、AI推論サービスで同じTraceを継続する。Trace Contextの具体的なHTTP表現は、採用するOpenTelemetry propagatorと通信実装・契約を正本とし、RAGScope独自の共通実行追跡IDを並行して追加しない。
...
RAGScopeアプリケーションとAI推論サービスは、OpenTelemetryを共通基盤としてTrace、Logs、Metricsを生成する。OpenTelemetryによる観測は、第2章で定めた処理制御やコンポーネント責務を変更しない。

ローカル環境では、TraceをTempo、LogsをLoki、MetricsをPrometheusで扱い、Grafanaから横断して確認する。

```mermaid
flowchart LR
    Application["RAGScopeアプリケーション"]
    AI["AI推論サービス"]
    Route["Telemetry送信経路<br>具体構成は配置時に決定"]
...
この図の`Telemetry送信経路`は論理的な境界を示す。OpenTelemetry Collectorを配置するか、各SDKからどのprotocolで送信するかという具体構成はここでは固定せず、ローカルObservability環境を実装する構成定義を正本とする。production環境のbackend、保持、冗長化も別の配置設計で定める。

Trace、Logs、Metricsの意味と使い分け、Span・EventRecord・`error.type`の共通規則は[Observability設計](./observability/README.md)を正本とする。
