...
| [ユースケース基本設計](./ユースケース基本設計.md) | 利用者のトップレベルな操作は何で、1回の実行はどこからどこまでか |
...
flowchart TD
    ...
    subgraph Core["RAGScope全体の設計"]
        UseCase["ユースケース基本設計<br>トップレベルな操作と1回の実行境界"]
...
利用者がRAGScopeへ何を依頼し、その1回の操作をどこまで一連の実行として扱うかは[ユースケース基本設計](./ユースケース基本設計.md)を確認する。その成功・最終失敗をRAGScopeアプリケーションのHaskell実装でどう表すかは[ユースケース詳細設計](./ユースケース詳細設計.md)を確認する。その処理をどのコンポーネントが担当し、AI推論サービスやデータベースとどう連携するかは[システムアーキテクチャ](./システムアーキテクチャ.md)を確認する。
