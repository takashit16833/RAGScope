...
| [ユースケース基本設計](./ユースケース基本設計.md) | 利用者のトップレベルな操作は何で、1回の実行はどこからどこまでか。何を成功・最終失敗とみなすか |
...
flowchart TD
    ...
    subgraph Core["RAGScope全体の設計"]
        UseCase["ユースケース基本設計<br>トップレベルな操作・実行境界<br>成功 / 最終失敗の意味"]
...
利用者がRAGScopeへ何を依頼し、その操作で**何が成立すれば成功で、何を最終失敗とみなすか**は[ユースケース基本設計](./ユースケース基本設計.md)を確認する。

そのユースケースを構成する各処理で**具体的にどのような失敗があるか**は[機能設計](./features/README.md)を確認する。それらの失敗を**Haskell上でどう表現し、ユースケースの最終失敗としてApplicationへどう受け渡すか**は[ユースケース詳細設計](./ユースケース詳細設計.md)を確認する。
