...
各UseCaseで**何を結果とし、どの条件をUseCaseの失敗とみなすか**と、それらを**Haskell上でどう表現してApplicationへ受け渡すか**は[ユースケース基本設計](./ユースケース基本設計.md)と[ユースケース詳細設計](./ユースケース詳細設計.md)を確認する。文書読み込み・検証・チャンク化という内部処理の具体的な規則と失敗条件は[文書処理設計](./文書処理設計.md)を確認する。
