...
| 文書 | 概要 |
|---|---|
| [RAGScope要求定義](../RAGScope要求定義.md) | RAGScopeが満たすべき要求を定義する |
| [システムアーキテクチャ](./システムアーキテクチャ.md) | システム全体のコンポーネント構成と責務を定義する |
| [文書処理設計](./文書処理設計.md) | Markdown文書の読み込みと文書チャンクへの変換を定義する |
| [共通エラー契約設計](./共通エラー契約設計.md) | 処理が成立しなかった理由をRAGScope全体で共有する契約を定義する |
| [実行追跡・構造化ログ契約設計](./実行追跡・構造化ログ契約設計.md) | 出来事と処理・ドメイン対象の関係をログから追跡する契約を定義する |
...
flowchart TD
    ...
    Logging["実行追跡・構造化ログ契約設計<br>出来事と処理・ドメイン対象の関係"]
