...
Operationはこの処理範囲を表す意味上の境界であり、UseCaseと1対1で対応する機能固有Application Serviceではない。利用インターフェースのhandlerが操作固有の処理順序を担当し、その全体をOperation境界で囲む。`SearchOperation`のような機能固有Operation moduleをUseCaseとは別に設けることは、この設計の前提にしない。
