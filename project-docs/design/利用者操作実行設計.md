...
>
> 個々のUseCaseと`UseCaseResult`は[ユースケース基本設計](./ユースケース基本設計.md)、HaskellのUseCase境界と具体failureの受け渡しは[ユースケース詳細設計](./ユースケース詳細設計.md)、具体的なfailureから共通`ErrorType`への変換は[失敗理由ErrorType変換設計](./失敗理由ErrorType変換設計.md)、利用者操作を`trace`・`span`で追跡する規則は[実行追跡・構造化ログ契約設計](./logging/実行追跡・構造化ログ契約設計.md)を正本とする。
...
UseCase前、UseCase、UseCase後で生じる具体的なfailureのうち、root `span`などで失敗理由を`error_type`として観測するfailureは、[失敗理由ErrorType変換設計](./失敗理由ErrorType変換設計.md)で定義する共通`ToErrorType`契約へ接続する。UseCase外のfailureをUseCase固有failureへ変換することはせず、それぞれの処理を所有する側の具体的なfailureとして扱う。

## 3. 現在固定していない実装詳細

この設計では、現時点で次を固定しない。これらは利用者操作全体の実行・失敗境界として設計が確定した時点で、この文書へ現在設計を追加する。

- `OperationResult`を表す正確なHaskell型名、constructor、module配置。
- `OperationFailure`がUseCase前・UseCase・UseCase後の通常失敗をどの型で保持するか。
- UseCase固有failureを`OperationFailure`へどう受け渡すかの正確なHaskell表現。
- 同期Exceptionを捕捉する境界と、通常の`OperationFailure`へ変換する条件。
- 非同期Exceptionのcleanupと再throwの境界。
- logging基盤自身のfailureを`OperationResult`へどう反映するか。
- `OperationFailure`とlogging failureが同時に発生した場合の最終結果と保持する失敗情報。

正確なHaskell型・constructor・関数定義が実装された場合は、コードとテストを機械可読な正本とし、この文書にはその意味、境界、受け渡し規則を記載する。
