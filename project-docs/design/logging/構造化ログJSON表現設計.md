...
> [!abstract] この文書の役割
> [実行追跡・構造化ログ契約設計](./実行追跡・構造化ログ契約設計.md)で定義する構造化ログ1件を、[構造化ログ外部表現共通設計](./構造化ログ外部表現共通設計.md)の共通情報と属性の規則を使ってJSONへどう配置するかを定義する。イベントや属性の意味・記録条件は論理契約と各機能設計を正本とし、この文書はJSON固有の階層、JSON上の値型、省略規則、出力単位を担当する。
...
- 各機能設計の記録条件を満たさず属性が存在しない場合は、そのpropertyを作らない。
...
具体的な出力先、buffering、IO失敗時の扱いは、各コンポーネントの構造化ログ設計が担当する。
...
| 各コンポーネントの構造化ログ設計 | JSON表現へ投影する境界、出力先、ログ基盤自身の失敗の扱い |
