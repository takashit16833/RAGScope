...
>
> 構造化ログの論理情報と記録規則は[実行追跡・構造化ログ契約設計](./実行追跡・構造化ログ契約設計.md)、具体的なfailureから共通`ErrorType`への変換は[失敗理由ErrorType変換設計](../失敗理由ErrorType変換設計.md)、具体的なeventの意味・記録条件・属性はそのeventを所有する設計、JSON・SQLiteへの投影は各外部表現設計を正本とする。
...
具体的なfailureを`error_type`として観測する場合は、[失敗理由ErrorType変換設計](../失敗理由ErrorType変換設計.md)の共通`ToErrorType`契約によって`ErrorType`へ変換する。
...
| `ErrorType` / `ToErrorType` | [失敗理由ErrorType変換設計](../失敗理由ErrorType変換設計.md)。具体的なfailureから観測用`ErrorType`へ変換する共通契約 |
