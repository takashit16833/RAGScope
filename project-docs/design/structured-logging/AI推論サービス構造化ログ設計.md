...
| 知りたいこと | 参照先 |
|---|---|
| この文書で使う共通ログ用語の意味 | [前提となる共通概念](#前提となる共通概念) |
...
本設計で使用する次の用語はRAGScope共通の構造化ログ概念であり、本設計では意味を再定義しない。用語の意味を確認したい場合は、[構造化ログ設計](./構造化ログ設計.md)の該当節を正本とする。

| 共通概念 | 意味の正本 |
|---|---|
| `LogEvent` / `Envelope` / `EventContext` / `EventSpec` / `Operation` / `Payload` | [構造化ログ設計「1. ログイベントの意味モデル」](<./構造化ログ設計.md#1. ログイベントの意味モデル>) |
| `NormalEvent` / `FailedEvent` / `NormalLevel` / `LogError` | [構造化ログ設計「2. 通常イベントと失敗イベント」](<./構造化ログ設計.md#2. 通常イベントと失敗イベント>) |
| `Execution` / `Service` / `execution_id` | [構造化ログ設計「3. 実行との関連付け」](<./構造化ログ設計.md#3. 実行との関連付け>) |
...
通常の機能処理は、任意の`Operation`、通常イベント名、ログレベル、`Payload`を直接組み立てず、各機能で定義した閉じたイベントを使用する。機能ログ定義が閉じたイベントを共通`EventSpec`へ変換し、共通ログ基盤は意味が確定した`EventSpec`以降を担当する。
...
| `EventContext` | ログ記録環境を構成する境界 | ExecutionまたはServiceの意味が確定した状態で保持する |

イベント識別子と記録時刻は機能処理から受け取らない。出力対象外のイベントでは、不要なイベント識別子の生成と時計の参照を行わない。

### 2.1 ExecutionとService

Executionに属するログ記録環境は、呼び出し元から引き継いだ`execution_id`を持つ`EventContext`を保持する。同じCLIコマンドに由来する処理について、AI推論サービス側で新しい`execution_id`を生成しない。

Serviceに属するログ記録環境は`execution_id`を持たない。ExecutionとServiceのどちらに属するかを、nullableな`execution_id`の有無や実行中に変更可能な共有状態で表現しない。
...
|---|---|---|
| 通常イベント | 通常イベント名、通常レベル | `LogError`、失敗専用のイベント名やレベル |
| 失敗イベント | 安全な`LogError` | 失敗時に固定的に決まるイベント名やレベルを独立した入力として重複保持する状態 |

通常イベントと失敗イベントは、同じレコードのnullable項目や真偽値で切り替えるのではなく、意味の異なる2種類として区別する。通常レベルとして表現できるのは共通設計で定めた通常イベント用のレベルだけとする。
...
共通設計上の`Envelope`は、契約バージョン・イベント識別子・記録時刻・生成コンポーネントをまとめた**意味上のグルーピング**として扱う。Pythonで独立した`Envelope`型を設けることは要求しない。
