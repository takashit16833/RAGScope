...
実行時には、機能処理で実際に起きた出来事や得られた結果に対応する機能イベントを生成し、必要な値を持たせてログ受付処理へ渡す。

各機能は、機能イベントの種類ごとに、対応する[`Operation`](<./構造化ログ設計.md#1. ログイベントの意味モデル>)、`EventKind`、[`Payload`](<./構造化ログ設計.md#1. ログイベントの意味モデル>)の作り方を定義する。通常イベントでは通常イベント名と通常ログレベルも定義し、失敗イベントでは安全な`LogError`を使用する。
