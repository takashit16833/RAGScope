...
機能ごとのログ変換処理がイベント値を受け取り、[`EventSpec`](<./構造化ログ設計.md#1. ログイベントの意味モデル>)を作る。

```mermaid
flowchart LR
    Feature["機能処理"]
    FeatureEvent["機能で定義した<br>イベント値"]
    Convert["機能ごとのログ変換処理"]
    EventSpec["EventSpec"]
    Record["ログ記録処理"]
...
ログ変換処理は、受け取ったイベント値に応じて次を決める。

- どの処理についての出来事かを表す`Operation`
- 通常イベントか失敗イベントかを表す`EventKind`
- その出来事について記録する`Payload`

通常イベントでは、イベント値に対応する通常イベント名と通常ログレベルもここで決める。失敗イベントでは、安全な`LogError`を使用する。

機能処理は`EventSpec`の各要素をログ記録のたびに組み立てない。同じ種類の機能イベントからどの`EventSpec`を作るかは、機能ごとのログ変換処理にまとめる。
