...
文書集合と評価データセットを使って実験を行い、各評価データについて、検索結果、回答生成用文書チャンク、回答、引用などを評価データごとの実行結果として追跡し、それらを評価して得た指標や判定を評価データごとの評価結果として追跡する。さらに、実験全体の集計結果を確認し、実験条件の違いによって結果がどう変わったかを比較できる。
...
- 評価データセットを使って実験を行い、評価データごとの実行結果、評価データごとの評価結果、実験全体の集計結果を記録できる。
...
実験全体の集計結果だけでなく、評価データごとの実行結果と評価データごとの評価結果を確認できる。検索結果、回答生成用文書チャンク、回答、引用、処理時間、状態、エラーと、それらを評価して得た指標や判定をたどり、どの段階で問題が生じたかを分析できる。
...
4. 評価データごとの実行結果を確認し、検索結果、回答生成用文書チャンク、回答、引用、処理時間、状態、エラーを追跡する。
5. 評価データごとの評価結果を確認し、評価で得た指標や判定を追跡する。
6. 実験全体の集計結果を確認する。
7. 条件の異なる実験を比較し、評価データごとの差や失敗例を分析する。
8. 分析結果に基づいて設計や設定を見直し、必要な条件で再び実験する。
...
flowchart TD
    ...
    Experiment --> Execution["評価データごとの実行結果"]
    Execution --> Evaluation["評価データごとの評価結果"]
    Execution --> Aggregate["実験全体の集計結果"]
    Evaluation --> Aggregate
    Execution --> Compare["比較・失敗分析"]
    Evaluation --> Compare
    Aggregate --> Compare
