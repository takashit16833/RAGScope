...
5. 利用インターフェースからの1回の呼び出しをInvocationとし、1 Invocationにつき独立した1 Traceを作る。APIはHTTP SERVER Span、CLIはExecutionのcallee Spanをentry Spanとし、Invocation専用Spanやhandler Spanは追加しない。RAGScopeアプリケーションからAI推論サービスへは同じTrace Contextを伝播する。外部callerからAPIへ渡されたTrace ContextはInvocation Traceのparentにしない。
