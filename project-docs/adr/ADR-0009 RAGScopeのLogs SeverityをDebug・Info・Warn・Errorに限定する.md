...
> [!note]
> 本ADRは[ADR-0010 — RAGScopeアプリケーションの失敗を処理単位の具体型で扱う](<./ADR-0010 RAGScopeアプリケーションの失敗を処理単位の具体型で扱う.md>)で置き換えられた。本ADRで決めた4段階SeverityはADR-0010で維持し、ADR-0006から引き継いでいた失敗の扱いをADR-0010で変更している。
